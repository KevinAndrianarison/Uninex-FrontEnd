      ...
      <div class="relative w-64 mt-5 text-xs">
        ...
        >
          <li
            ...
            :key="index"
            @mousedown="selectOption(item.nomComplet, item.user.id)"
...
function deleteConge(id) {
  ...
  axios
    ...
    .then((response) => {
      conge.getAllCongepermissionById(idUser.value)
...
function postDemande() {
  ...
  formData.append('user_id', Number(idUser.value) || null)
...
const selectOption = (nom, id) => {
  nomUser.value = nom
  searchQuery.value = nom
  idUser.value = id
  conge.getAllCongepermissionById(id)
  isOpen.value = false
}
